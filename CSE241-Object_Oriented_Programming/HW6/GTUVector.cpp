...
namespace _GTU_HW_6_{
	...
	template<class T>
	GTUVector<T>::GTUVector(int n,T value){
		if(MAX_SIZE < n)
			exit(1);
	...
	template<class T>
	const GTUIterator<T> GTUVector<T>::erase(const GTUIterator<T> &position){
	...
	template<class T>
	const GTUIterator<T> GTUVector<T>::erase(const GTUIterator<T> &first,const GTUIterator<T> &last){
		if(first==last)
			return HeadOfContent;
	...
	template<class T>
	const GTUIterator<T> GTUVector<T>::insert(T val){
		if(_size >= MAX_SIZE){
	...
	template<class T>
	const GTUIterator<T> GTUVector<T>::insert(const GTUIterator<T> &position, T val){
		if(_size >= MAX_SIZE){
			GTUExceptions("MAX_SIZE problem!");
		}
	...
	template<class T>
	const T* GTUVector<T>::operator->()const{
		return &(HeadOfContent->data);
	}
